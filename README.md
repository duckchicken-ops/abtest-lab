...
A/B 테스트에서 간과하기 쉬운 통계적 오류를 시뮬레이션으로 검증하는 도구.
...
세 경우 모두 두 그룹에 실제 차이가 없다. 각 수치는 1,000회 시뮬레이션으로
측정했으며 `notebooks/`에서 재현할 수 있다.

대응 기법도 함께 다룬다. 순차검정을 적용하면 30일 매일 확인해도 위양성률이
0.7%로 유지되며 검정력 손실은 1%p에 그친다(`02_sequential`). CUPED로 분산을
절반으로 줄이면 같은 데이터에서 p값이 0.0713에서 0.0039로 개선된다(`05_cuped`).

## 왜 만들었나

A/B 테스트 결과를 판정하는 과정을 학습하면서, 두 그룹에 실제 차이가 없는 데이터에서
5%p 차이가 관측되는 경우를 반복해서 만났다. 오류도 경고도 없이
그럴듯한 숫자가 나오기 때문에 코드를 검토해도 발견되지 않는다.

이런 실패는 세 가지 경로로 발생한다. 세그먼트를 여러 개 확인하거나,
실험 중간에 반복해서 들여다보거나, 배정 비율이 어긋난 경우다.

각 상황의 위험성을 시뮬레이션으로 정량화하고,
실무에서 이를 피하기 위한 절차를 실험 설계 문서로 정리했다.
...
노트북에서 위험 요소를 확인했다면, 아래 문서는 그것을 피하는 절차를 담는다.
